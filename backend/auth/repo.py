...
class AuthRepo:
    ...
    async def save_profile(self, new_profile: Profile) -> Profile:
        ...
        try:
            result = await db.fetch_one(
                insert(profile)
                .values(new_profile.dict(exclude_none=True))
                .returning(profile)
            )
    ...
    async def find_profile_by_email(self, email: str) -> Optional[Profile]:
        return await db.fetch_one(
            select([profile]).where(profile.c.email == email.lower())
        )

    @map_result
    async def find_jwt_refresh_token(self, token_id: UUID) -> Optional[JwtRefreshToken]:
        return await db.fetch_one(
            select([jwt_refresh_token]).where(jwt_refresh_token.c.id == token_id)
        )
    ...
    async def update_jwt_refresh_token(
        self, token_id: UUID, values: Dict
    ) -> JwtRefreshToken:
        return await db.fetch_one(
