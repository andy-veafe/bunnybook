...
class NotificationRepo:
    @map_result
    async def save_notification(self, new_notification: Notification) -> Notification:
    ...
    async def update_notifications_read_visited_status(
        ...
        return await db.fetch_all(
            update(notification)
            .where(notification.c.id.in_([literal(n) for n in notification_ids]))
            .values(
                **(dict(read=read) if read is not None else {}),
                **(dict(visited=visited) if visited is not None else {})
            )
            .returning(notification)
        )
