...
class ChatService(WsRouter):
    ...
    async def get_conversations(
        ...
        return await self._repo.find_conversations_by_profile_id(
            profile_id, older_than, limit
        )

    async def get_chat_group_members_profile_ids(
        self, chat_group_id: UUID
    ) -> List[UUID]:
        """Return ids of a chat group members."""
        return await self._repo.find_chat_group_members_profile_ids(chat_group_id)

    async def get_private_chats_by_profile_id(
    ...
    async def _on_chat_message(self, sid: str, data: Dict):
        ...
        if to not in [chat.chat_group_id for chat in session.private_chats]:
            return
        chat_message = ChatMessage(
            from_profile_id=session.user.id, chat_group_id=to, content=message
        )
        chat_message = await self._repo.save_chat_message(chat_message)
        await self._repo.update_chat_message_read_status(
            session.user.id, to, chat_message.id
        )
    ...
    async def _on_mark_chat_as_read(self, sid: str, data: Dict):
        chat_group_id, chat_message_id = data["chatGroupId"], data["chatMessageId"]
        session = await get_sio_session(sid)
        await self._repo.update_chat_message_read_status(
            session.user.id, chat_group_id, chat_message_id
        )
    ...
    async def _get_message_recipients(
        ...
        return [
            str(profile_id)
            for profile_id in await self._repo.find_chat_group_members_profile_ids(
                chat_group_id
            )
            if (
                True
    ...
    async def _on_ws_connect(self, sid: str, user: User):
        ...
        unread_conversations_ids = (
            await self._repo.find_unread_conversations_ids_by_profile_id(user.id)
        )
        await save_sio_session(sid, SioSession(user=user, private_chats=private_chats))
        await self._ws.send(
            event="unread_conversations_ids", payload=unread_conversations_ids, to=sid
        )
        await self._ws.send(
            event="private_chats",
    ...
    async def _online_friends_task(self, sid: str):
        while True:
            try:
                ...
                return
            friends_uuids = [friend.profile_id for friend in session.private_chats]
            await self._ws.send(
