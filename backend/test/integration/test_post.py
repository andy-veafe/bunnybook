...
async def test_create_post(ben):
    ...
    new_post_req = await ben.conn.post("/posts", json={"content": new_post_content})
...
async def test_post_privacy(ben, daisy):
    public_post = (
        await ben.conn.post("/posts", json={"content": "public", "privacy": "PUBLIC"})
    ).json()
    friends_post = (
        await ben.conn.post("/posts", json={"content": "friends", "privacy": "FRIENDS"})
    ).json()
    assert (
        (await ben.conn.get(f"/posts/{public_post['id']}")).status_code,
        (await ben.conn.get(f"/posts/{friends_post['id']}")).status_code,
    ) == (200, 200)
    assert (
        (await daisy.conn.get(f"/posts/{public_post['id']}")).status_code,
...
async def test_get_many_posts(ben, daisy):
    ...
    posts = (await daisy.conn.get("/posts", params={"wall_profile_id": ben.id})).json()
    assert len(posts) == 1
    await become_friends(ben, daisy)
    posts = (await daisy.conn.get("/posts", params={"wall_profile_id": ben.id})).json()
    assert len(posts) == 2
...
async def test_post_delete_on_another_profile_wall(ben, daisy):
    ...
    post_id = (
        await ben.conn.post(
            "/posts", json={"content": "Test", "wall_profile_id": daisy.id}
        )
    ).json()["id"]
    assert (await ben.conn.delete(f"/posts/{post_id}")).status_code == 204
    post_id = (
...
async def test_post_privacy_on_another_profile_wall(ben, daisy, sumba, pumba):
    ...
    public_post = await ben.conn.post(
        "/posts",
        json={"content": "Test", "wall_profile_id": daisy.id, "privacy": "PUBLIC"},
    )
    public_post_id = public_post.json()["id"]
    friends_post = await ben.conn.post(
        "/posts",
        json={"content": "Test", "wall_profile_id": daisy.id, "privacy": "FRIENDS"},
    )
    friends_post_id = friends_post.json()["id"]
    assert (await sumba.conn.get(f"/posts/{public_post_id}")).status_code == 200
    assert (await sumba.conn.get(f"/posts/{friends_post_id}")).status_code == 403
    await become_friends(sumba, ben)
    await become_friends(pumba, daisy)
    assert (await sumba.conn.get(f"/posts/{friends_post_id}")).status_code == 403
    assert (await pumba.conn.get(f"/posts/{friends_post_id}")).status_code == 200
