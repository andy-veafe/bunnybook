...
class PostApi:
    ...
    _notifications: PostNotificationService = Depends(on(PostNotificationService))
    ...
    async def create_post(
        ...
        post = await self._service.save_post(
            Post(**post_in.dict(), profile_id=user.id, username=user.username)
        )
    ...
    async def get_post_by_id(
        self, post_id: UUID, user: Optional[User] = Depends(get_optional_user)
    ):
    ...
    async def delete_post(self, post_id: UUID, user: User = Depends(get_user)):
