...
class PostApiUtils:
    @inject
    def __init__(self, service: PostService, profiles_service: ProfilesService):
        self._service = service
        self._profiles_service = profiles_service

    async def check_user_can_see_post(
        self, user: Optional[User], post_id: UUID
    ) -> Post:
        ...
        if (
            not user
            and post.privacy == PostPrivacy.FRIENDS
            or post.privacy == PostPrivacy.FRIENDS
            and user
            and user.id != post.wall_profile_id
