...
class PostRepo:
    ...
    async def save_post(self, new_post: Post) -> Post:
        saved_post = await db.fetch_one(
            insert(post)
            .values(new_post.dict(exclude_none=True, exclude={"username"}))
            .returning(post)
        )
        saved_post: Post = map_to(saved_post, Post)
        saved_post.username = new_post.username
        await self._cache.unset_posts_ids(new_post.wall_profile_id, True, None)
        if new_post.privacy == PostPrivacy.PUBLIC:
            await self._cache.unset_posts_ids(new_post.wall_profile_id, False, None)
        return saved_post

    @map_result
    async def delete_post(self, post_id: UUID) -> Optional[Post]:
        deleted_post = await db.fetch_one(
            delete(post).where(post.c.id == post_id).returning(post)
        )
    ...
    async def find_posts_by_wall_profile_id(
        ...
        if (
            posts := await self._cache.get_posts(
        ...
        in_clause_values = [
            privacy
            for privacy, should_include in zip(
                [literal(PostPrivacy.PUBLIC.value), literal(PostPrivacy.FRIENDS.value)],
                [True, include_friends],
            )
            if should_include
        ]
        ...
        await self._cache.set_posts(posts, wall_profile_id, include_friends, older_than)
        return posts
