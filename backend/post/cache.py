...
class PostCache:
    ...
    @fail_silently()
    async def get_posts(
        self, wall_profile_id: UUID, include_friends: bool, older_than: dt.datetime
    ) -> Optional[List[Post]]:
        ...
        return (all(cached_posts) or None) and [
            map_to(json.loads(post), Post) for post in cached_posts
        ]
    ...
    async def set_posts(
        ...
        older_than: Optional[dt.date],
    ) -> None:
        params_cache_key = hash_cache_key(wall_profile_id, include_friends, older_than)
        ...
        pipe.mset(
            ...
            *list(
                sum(
                    [
                        (f"posts:{post.id}", json.dumps(jsonable_encoder(post)))
                        for post in posts
                    ],
                    (),
                )
