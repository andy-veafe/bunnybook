...
class CommentCache:
    ...
    async def get_comments(
        self, post_id: UUID, older_than: dt.datetime
    ) -> Optional[List[Comment]]:
        cached_comments = await self._cache.get(
            f"posts:{post_id}:comments:{hash_cache_key(older_than)}"
        )
        return cached_comments and map_to(json.loads(cached_comments), List[Comment])

    @fail_silently()
    async def set_comments(
        self, comments: List[Comment], post_id: UUID, older_than: Optional[dt.date]
    ) -> None:
    ...
    async def unset_latest_comments(self, post_id: UUID) -> None:
        await self._cache.delete(f"posts:{post_id}:comments:{hash_cache_key(None)}")
