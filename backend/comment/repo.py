...
class CommentRepo:
    ...
    async def save_comment(self, new_comment: Comment) -> Comment:
        ...
        saved_comment = await db.fetch_one(
            insert(comment)
            .values(new_comment.dict(exclude_none=True, exclude={"username"}))
            .returning(comment)
        )
    ...
    async def find_comments_by_post_id(
        self, post_id: UUID, older_than: Optional[dt.datetime] = None, limit: int = 10
    ) -> List[Comment]:
        older_than_clause = older_than or dt.datetime.now(dt.timezone.utc)
        if (
            comments := await self._cache.get_comments(post_id, older_than)
        ) is not None:
            return comments
    ...
    async def find_comments_authors_by_post_id(self, post_id: UUID) -> Set[UUID]:
        results = await db.fetch_all(
            select([comment.c.profile_id.distinct()]).where(
