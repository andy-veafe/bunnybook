...
class ProfilesService:
    ...
    async def find_profile_by_profile_id(self, profile_id: UUID) -> ProfileShort:
    ...
    async def create_friend_request(
        ...
        await self._check_relationship(
            requester_profile_id, target_profile_id, [Relationship.NONE]
        )
        await self._repo.save_friend_request(requester_profile_id, target_profile_id)
    ...
    async def reject_incoming_friend_request(
        self, requester_profile_id: UUID, rejecter_profile_id: UUID
    ) -> None:
        ...
        await self._repo.delete_friend_request(
            from_profile_id=requester_profile_id, to_profile_id=rejecter_profile_id
        )

    async def cancel_outgoing_friend_request(
        self, canceler_profile_id: UUID, target_profile_id: UUID
    ) -> None:
        ...
        await self._repo.delete_friend_request(
            from_profile_id=canceler_profile_id, to_profile_id=target_profile_id
        )

    async def remove_friend(self, profile_id: UUID, friend_profile_id: UUID) -> None:
        """Delete friendship relationship between 'profile_id' and
        'friend_profile_id'."""
        await self._check_relationship(
            profile_id, friend_profile_id, [Relationship.FRIEND]
        )
        await self._repo.delete_friend(profile_id, friend_profile_id)
        for recipient in [profile_id, friend_profile_id]:
            await self._ws.send(
                event="remove_friend",
                payload=profile_id if recipient != profile_id else friend_profile_id,
                to=recipient,
            )
    ...
    async def find_friend_suggestions(
        ...
        return await self._repo.find_friends_of_friends(profile_id, username_gt, limit)
    ...
    async def is_friend_with(self, profile_id: UUID, other_profile_id: UUID) -> bool:
        """Determine whether 'profile_id' and 'other_profile_id' are friends or
        not."""
        return (
            profile_id == other_profile_id
            or Relationship.FRIEND
            == await self._repo.find_relationship(profile_id, other_profile_id)
        )
    ...
    async def _check_relationship(
        ...
        if (
            await self.find_relationship(profile_id, other_profile_id)
