...
class ProfilesRepo:
    @inject
    def __init__(
        self, cache: ProfilesCache, graph_db: AsyncGraphDatabase, chat_repo: ChatRepo
    ):
        self._cache = cache
    ...
    async def find_profiles_by_username_search(
        ...
        return await db.fetch_all(
            select([profile.c.id, profile.c.username])
            .where(profile.c.username.ilike(f"%{username}%"))
            .limit(limit)
            .order_by(desc(profile.c.username.ilike(f"{username}%")))
        )

    @map_result
    async def find_profile_by_id(self, profile_id: UUID) -> Optional[ProfileShort]:
        return await db.fetch_one(select([profile]).where(profile.c.id == profile_id))

    @map_result
    async def find_profiles_by_ids(self, profile_ids: List[UUID]) -> List[ProfileShort]:
        return await db.fetch_all(
            select([profile]).where(
                profile.c.id.in_([literal(profile_id) for profile_id in profile_ids])
            )
        )

    async def save_friend_request(
        self, requester_profile_id: UUID, target_profile_id: UUID
    ) -> None:
        await self._graph_db.write_tx(
            lambda tx: tx.run(
                f"""
        ...
        await self._cache.unset_relationship(requester_profile_id, target_profile_id)

    @db.transaction()
    async def save_friendship_relationship(
        self, requester_profile_id: UUID, accepter_profile_id: UUID
    ) -> ChatGroup:
        ...
        await self._cache.unset_relationship(
            requester_profile_id, accepter_profile_id, delete_cached_friends=True
        )
        return chat_group

    async def delete_friend_request(
        self, from_profile_id: UUID, to_profile_id: UUID
    ) -> None:
        await self._graph_db.write_tx(
            lambda tx: tx.run(
                f"""
    ...
    @db.transaction()
    async def delete_friend(self, profile_id: UUID, friend_profile_id: UUID) -> None:
        chat_group = await self._chat_repo.find_private_chat_group(
        ...
        await self._cache.unset_relationship(
            profile_id, friend_profile_id, delete_cached_friends=True
        )

    async def find_friends(
        self, profile_id: UUID, username_gt: Optional[str] = None, limit: int = 10
    ) -> List[ProfileShort]:
        if friends := await self._cache.get_friends(profile_id):
            return friends
        friends = await self._graph_db.read_tx(
            lambda tx: list(
                tx.run(
                    f"""
    ...
    async def find_friends_of_friends(
        ...
        return await self._graph_db.read_tx(
            lambda tx: list(
                tx.run(
                    f"""
        MATCH (profile:Profile {{id: '{profile_id}'}})
    ...
    async def find_relationship(
        ...
        if relationship := await self._cache.get_relationship(
            profile_id, other_profile_id
        ):
            return Relationship(relationship)
        if profile_id == other_profile_id:
            return Relationship.SELF
        result = await self._graph_db.read_tx(
            lambda tx: list(
                tx.run(
                    f"""
        MATCH (profile:Profile {{id: '{profile_id}'}})\
        ...
        await self._cache.set_relationship(profile_id, other_profile_id, relationship)
        return relationship
