...
class ProfilesCache:
    ...
    @fail_silently()
    async def get_relationship(self, profile_id: UUID, other_profile_id: UUID) -> str:
        min_id, max_id = min(profile_id, other_profile_id), max(
    ...
    async def set_relationship(
        self, profile_id: UUID, other_profile_id: UUID, relationship: Relationship
    ) -> str:
    ...
    async def unset_relationship(
        ...
        delete_cached_friends: bool = False,
    ) -> int:
        min_id, max_id = min(profile_id, other_profile_id), max(
            profile_id, other_profile_id
        )
        return await self._cache.delete(
            ...
            *(
                ...
                if delete_cached_friends
                else []
            ),
        )

    @fail_silently()
    async def get_friends(self, profile_id: UUID) -> Optional[List[ProfileShort]]:
        json_friends = await self._cache.get(f"profiles:{profile_id}:friends")
        return (
            map_to(json.loads(json_friends), List[ProfileShort])
            if json_friends
            else None
        )

    @fail_silently()
    async def set_friends(self, profile_id: UUID, friends: List[ProfileShort]) -> None:
        return await self._cache.set(
            f"profiles:{profile_id}:friends",
    ...
    async def unset_friends(self, profile_ids: List[UUID]) -> None:
        return await self._cache.delete(
            *[f"profiles:{profile_id}:friends" for profile_id in profile_ids]
        )
