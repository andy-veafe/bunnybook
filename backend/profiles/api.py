...
from fastapi import Depends, HTTPException, Query, BackgroundTasks, status
...
class ProfilesApi:
    ...
    async def create_friend_request(
        ...
        await self._service.create_friend_request(
            requester_profile_id=profile_id, target_profile_id=target_profile_id
        )
    ...
    async def accept_friend_request(
        ...
        await self._service.accept_friend_request(
            requester_profile_id=requester_profile_id, accepter_profile_id=profile_id
        )
        background_tasks.add_task(
    ...
    async def reject_incoming_friend_request(
        ...
        return await self._service.reject_incoming_friend_request(
            requester_profile_id=requester_profile_id, rejecter_profile_id=profile_id
        )
    ...
    async def cancel_outgoing_friend_request(
        self, profile_id: UUID, target_profile_id: UUID, user: User = Depends(get_user)
    ):
        """Cancel an outgoing friend request."""
        if user.id != profile_id:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN)
        return await self._service.cancel_outgoing_friend_request(
            canceler_profile_id=user.id, target_profile_id=target_profile_id
        )
    ...
    async def remove_friend(
        self, profile_id: UUID, friend_profile_id: UUID, user: User = Depends(get_user)
    ):
    ...
    async def get_friend_suggestions(
        ...
        if user.id != profile_id:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN)
        return await self._service.find_friend_suggestions(user.id, username_gt, limit)
    ...
    async def get_friend_requests(
        self,
        profile_id: UUID,
        direction: Optional[Literal["incoming", "outgoing"]] = Query("outgoing"),
        username_gt: Optional[str] = None,
        limit: Optional[int] = Query(20, ge=1, le=50),
        user: User = Depends(get_user),
    ):
    ...
    async def get_relationship(self, profile_id: UUID, other_profile_id: UUID):
        ...
        return await self._service.find_relationship(profile_id, other_profile_id)
